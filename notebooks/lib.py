...
# Count 'failures' where a model failed to converge and got stuck at a certain spot or whose loss exploded.
# Some useful masks
# Activation Function
...
# Dataset
using_535 = df['dataset'] == '535'
using_encoder = df['dataset'] == 'encoder'
using_iris = df['dataset'] == 'iris'
using_parity_3_bit = df['dataset'] == 'parity-3-bit'
using_parity_4_bit = df['dataset'] == 'parity-4-bit'
using_xor = df['dataset'] == 'xor'

# Shuffling Batches
using_batch_shuffling = df['shuffle_batches'] == True
...
def make_boxplot(df, base_mask, variable_mask, a_config, b_config, metric, title):    
    ...
    :param a_config: A string representation of the opposite configuration of the variable from variable_mask 
