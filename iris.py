import matplotlib.pyplot as plt
import numpy as np
from sklearn import utils

from mlp.activation_functions import Sigmoid, Identity
from mlp.datasets import load_iris
from mlp.layers import DenseLayer
from mlp.losses import RMSE
from mlp.network import MLPRegressor, EarlyStopping

if __name__ == '__main__':
    np.random.seed(42)

    X, y = load_iris()
    X, y = utils.shuffle(X, y, random_state=42)

    n_inputs = X.shape[1]
    mlp = MLPRegressor([DenseLayer(n_inputs, n_inputs=n_inputs, activation_func=Sigmoid()),
                        DenseLayer(3, activation_func=Identity())],
                       learning_rate=0.1,
                       momentum=0.99,
                       loss_func=RMSE())

    _, _, loss_history, _ = mlp.fit(X, y, n_epochs=10000, batch_size=2, shuffle_batches=True,
                                    early_stopping=EarlyStopping(patience=1000), log_verbosity=100)
    print("Targets: %s - Predictions: %s" % (y.ravel(), mlp.predict(X).ravel()))
    print("Score: %.4f" % mlp.score(X, y))

    plt.plot(loss_history)
    plt.show()
